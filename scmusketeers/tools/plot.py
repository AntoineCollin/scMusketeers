...
import anndata
import random
from .utils import ann_subset, dict_to_df, densify
import functools
...
colors = ["lightgray","#FFF7EC", "#FEE8C8", "#FDD49E",
          "#FDBB84", "#FC8D59", "#EF6548", "#D7301F", "#B30000", "#7F0000"]

cmap = mcolors.LinearSegmentedColormap.from_list("my_palette", colors)
sc.pl.umap = functools.partial(sc.pl.umap, size = 5, legend_fontsize = 'xx-small', legend_loc = 'on data', color_map = cmap) # setting default umap params
...
def quick_view(adata, groupby = None):
    ...
    sc.pl.violin(adata=adata, keys=['n_genes_by_counts','total_counts', 'pct_counts_mt', 'pct_counts_ribo'], jitter=0.2, multi_panel=True)
    sc.set_figure_params(dpi=70, figsize=(6,4))
    sc.pl.scatter(adata, x='total_counts', y='n_genes_by_counts')
    sc.pl.scatter(adata, x='n_genes_by_counts', y='pct_counts_mt', color = groupby)
    sc.pl.umap(adata,color=[groupby,'n_genes_by_counts','total_counts', 'pct_counts_mt', 'pct_counts_ribo'], legend_loc = 'on data')
    if groupby :
        sc.pl.violin(adata, keys=['n_genes_by_counts'], groupby = groupby, rotation=90)
        sc.pl.violin(adata, keys=['total_counts'], groupby = groupby, rotation=90)
        sc.pl.violin(adata, keys=['pct_counts_mt'], groupby = groupby, rotation=90)
        sc.pl.violin(adata, keys=['pct_counts_ribo'], groupby = groupby, rotation=90)
...
def violin(adata, subset, gene, group_by, hue,  figsize=(5,3)):
    ...
    to_plot[gene] = pd.Series(np.array(adata[to_subset,gene].X.todense()).ravel(), index = adata[to_subset,:].obs_names)
...
def plot_average_gene_list(adata,gene_list,ret_score = False, **kwargs):
    ...
    to_plot = sc.AnnData(X = meta_gene, obs = adata.obs, var = pd.DataFrame(index = ['mean_expression']), obsm = adata.obsm)
    if ret_score: 
        return to_plot
...
def umap_with_ncells(adata, color, **kwargs):
    ...
    celltype_sizes = {celltype: celltype + ' (n = ' + str(add_space(celltype_counts[celltype])) + ')' for celltype in celltype_counts.index}
...
def feature_umap_subset(adata, obs_key, subset, gene, **kwargs):
    ...
    sc.pl.umap(ann_subset(adata, obs_key, subset), color=gene, ax=ax, s=dot_size)
    return ax
...
def stackbar(y: np.ndarray,type_names: List[str],
        title: str,
    ...
    for n in range(n_types):
        ...
        plt.barh(r, height=barwidth,width=bars, left=cum_bars,  label=type_names[n], linewidth=0, color = cmap[n])
        cum_bars += bars
...
def plot_umap_jitter(adata, color,fontsize=10, **kwargs):
    ...
    ax = sc.pl.umap(adata, color=color, show=False, legend_loc=None, frameon=False,**kwargs)
    gen_mpl_labels(
        adata,
        color,
...
def pbulk_heatmap(pbulk, markers, genes_ct_level,split_by,
                  layer = None,
                  gene_th= 0, 
                  cells_th= 0,
                  title = None, 
    ...
    def get_color_dict(adata, color):
        if f'{color}_colors' in adata.uns:
            return dict(zip(adata.obs[color].cat.categories, adata.uns[f'{color}_colors']))
        else : 
            return None
    colors = {}
    ...
    annot_obs = pbulk.obs.loc[:,split_by + [genes_ct_level]] #.astype(str) # Selecting bar legends ['celltype_lv0_V6']
    annot_obs = annot_obs.sort_values([genes_ct_level] + split_by ) # Sorting bar legend order + ['celltype_lv0_V6']
    # Setting genes annotation
    gene_df = dict_to_df(markers, val_name = 'genes', key_name=genes_ct_level)
    gene_ct_map = {v: k for v,k in zip(gene_df['genes'], gene_df[genes_ct_level])}
    if not layer:
        X = pbulk.X
    else :
    ...
    annot_var[genes_ct_level] = pd.Categorical(annot_var[genes_ct_level], 
                                               categories = pbulk.obs[genes_ct_level].cat.remove_unused_categories().cat.categories) # Sorting genes in the order of the celltypes
    #print(annot_var)
    # annot_var = annot_var.sort_values(genes_ct_level) 

    # Defining heatmap dataframe
    expr_df = pd.DataFrame(densify(X), index = pbulk.obs_names, columns = pbulk.var_names)
    # expr_df = expr_df.loc[:,(expr_df>gene_th).any(axis = 0)]# # Removing unexpressed genes
    # expr_df = expr_df.loc[(expr_df>cells_th).any(axis = 1),:]#

    expr_df = expr_df.loc[annot_obs.index,annot_var['genes']] # Ordering expr matrix to be coherent with annotation
    ...
    top_ann = pch.HeatmapAnnotation(#celltype_lv0_V6 = pch.anno_simple(annot_obs['celltype_lv0_V6'].cat.remove_unused_categories(),add_text=False,height=4,legend=legend,
                                    ...
                                    **{split:pch.anno_simple(annot_obs[split],add_text=False,height=4,legend=legend, colors = colors[split],
                                                    legend_kws={'fontsize':10, 'color_text': False}) for split in split_by[::-1]},
                               # donor=pch.anno_simple(annot_obs[split_by],add_text=False,height=4,legend=legend, colors = colors[split_by],
                               #                      legend_kws={'fontsize':10, 'color_text': False}),
                                **{genes_ct_level:pch.anno_simple(annot_obs[genes_ct_level].cat.remove_unused_categories(), add_text=False,height=4,legend=True,
                                                            colors = {ct : c for ct, c in colors[genes_ct_level].items() if ct in annot_obs[genes_ct_level].cat.remove_unused_categories().cat.categories},
                                                        legend_kws={'fontsize':10, 'color_text': False})},                            
                                legend=legend,legend_gap=5,hgap=0.5)
    ...
    cm = pch.ClusterMapPlotter(data=expr_df,top_annotation=top_ann,right_annotation=right_ann,#left_annotation= left_ann,
                               # col_cluster=False,
                               # row_cluster=False,
