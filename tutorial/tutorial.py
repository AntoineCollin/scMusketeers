...
def create_tuto_data(
    ...
    sc.pl.umap(adata_test, color=class_key)
    ...
    path_new_adata = os.path.join(
        working_dir,"data", f"{name}-unknown-{sampling_percentage}.h5ad"
    )
    ...
    path_new_adata = os.path.join(
        working_dir,"data", f"{name}-ref-batch-{sampling_percentage}.h5ad"
    )
    adata_ref.write_h5ad(path_new_adata)
    path_new_adata = os.path.join(
        working_dir,"data", f"{name}-query-batch-{sampling_percentage}.h5ad"
    )
