...
#	$(ENV_PREFIX)poetry run flake8 scpermut/
	...
	$(ENV_PREFIX)poetry run black -l 79 --check scpermut/ tests/
	$(ENV_PREFIX)poetry run mypy --ignore-missing-imports scpermut/ tests/

.PHONY: test
test:             ## Run tests and generate coverage report.
	$(ENV_PREFIX)poetry run pytest -v -l --tb=short --maxfail=1 tests/
...
ci:          ## Create a new tag for continuous integration.
	@echo "WARNING: You need first to add changes to git (git add)"
