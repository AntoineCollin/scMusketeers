...
class ContrastiveAutoencoder(Autoencoder):
    ...
    def build_output(self):
        self.similarity = Input(shape=(1,), name = 'similarity', dtype=bool)
        self.loss = ContrastiveLoss(similarity = self.similarity, margin = self.contrastive_margin)
        ...
        self.extra_models['mean_norm'] = Model(inputs=self.input_layer, outputs=mean)
        self.extra_models['decoded'] = Model(inputs=self.input_layer, outputs=self.decoder_output)
        self.model = Model(inputs=[self.input_layer, self.sf_layer, self.similarity0], outputs=output)
...
class ZINBAutoencoder(Autoencoder):
    ...
    def write(self, adata, file_path, mode='denoise', colnames=None):
        ...
        if 'X_dca_dispersion' in adata.obsm_keys():
            write_text_matrix(np.array(adata.obsm['X_dca_dispersion']),
