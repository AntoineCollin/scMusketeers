...
def rand(adata, reference, partition_key, n_threshold = 70000):
    ...
    if adata.n_obs > n_threshold:
        l = []
        for i in range(100): # We compute ARI on 100 different samples of the dataset and return the average
            ind = np.random.choice(adata.obs_names, n_threshold)
            l.append(adjusted_rand_score(adata.obs[reference][ind],adata.obs[partition_key][ind]))
        return np.mean(l)
    else:
        return adjusted_rand_score(adata.obs[reference],adata.obs[partition_key])
...
def nmi(adata, reference, partition_key):
    ...
    """
    return adjusted_mutual_info_score(adata.obs[reference],adata.obs[partition_key])
...
import scipy
def batch_entropy_mixing(adata, batch_key, use_rep = None,  sub_key=None, sub_obs =None):
    if use_rep:
    ...
    if sub_obs and sub_key:
        return batch_entropy_mixing_score(data, adata.obs[batch_key], sub_population = np.where(adata.obs[sub_key] == sub_obs)[0])
    else :
