...
class Workflow:
    ...
    def set_hyperparameters(self, params):
        ...
        self.run_file.hp_params = params
            
    
    def process_dataset(self):
        # Loading dataset
        adata = load_dataset(dataset_dir = self.data_dir,
                               dataset_name = self.run_file.dataset_name)
        ...
        self.dataset = Dataset(adata = adata,
                               ...
                               size_factor = self.run_file.size_factor,
                               scale_input = self.run_file.scale_input,
                               logtrans_input = self.run_file.logtrans_input,
                               use_hvg = self.run_file.use_hvg,
    ...
    def split_train_test(self):
        self.dataset.test_split(test_obs = self.run_file.test_obs, test_index_name = self.run_file.test_index_name)
    
    def split_train_val(self):
        self.dataset.train_split(mode = self.run_file.mode,
                            pct_split = self.run_file.pct_split,
                            obs_key = self.run_file.obs_key,
                            n_keep = self.run_file.n_keep,
                            keep_obs = self.run_file.keep_obs,
                            split_strategy = self.run_file.split_strategy,
                            obs_subsample = self.run_file.obs_subsample,
    ...
    def make_experiment(self):
        if self.run_file.layer1 :
            self.ae_param.ae_hidden_size = [self.run_file.layer1, self.run_file.layer2, 
                                            self.run_file.bottleneck, self.run_file.layer2, 
        ...
        self.optimizer = self.get_optimizer(self.run_file.learning_rate, self.run_file.weight_decay, self.run_file.optimizer_type)
        ...
        # Training
        history = self.train_scheme(training_scheme=self.run_file.training_scheme,
                                    verbose = False,
                                    ae = self.dann_ae,
            ...
            for group in ['full', 'train', 'val', 'test']:
                with tf.device('CPU'):
                    ...
                    if group == 'full': # saving full predictions as probability output from the classifier
                        ...
                        self.run_neptune[f'evaluation/{group}/y_pred_proba_full'].track_files(save_dir + f'y_pred_proba_full.csv')
                    ...
                    self.run_neptune[f'evaluation/{group}/confusion_matrix_file'].track_files(save_dir + f'confusion_matrix_{group}.csv')
                    size = len(labels)
                    ...
                    if len(np.unique(np.asarray(batch_list[group].argmax(axis=1)))) >= 2: # If there are more than 2 batches in this group
                        for metric in BATCH_METRICS_LIST:
                            self.run_neptune[f'evaluation/{group}/{metric}'] = BATCH_METRICS_LIST[metric](enc, batches)
                            print(type(self.batch_metrics_list[metric](enc, batches)))
                            
                    # Computing classification metrics
                    for metric in PRED_METRICS_LIST: 
                        self.run_neptune[f"evaluation/{group}/{metric}"] = BATCH_METRICS_LIST[metric](y_true, y_pred)

                    for metric in PRED_METRICS_LIST_BALANCED: 
                        self.run_neptune[f"evaluation/{group}/{metric}"] = PRED_METRICS_LIST_BALANCED[metric](y_true, y_pred)
                    ...
                    for s in sizes : 
                        ...
                        # print(s)
                        for metric in PRED_METRICS_LIST: 
                            self.run_neptune[f"evaluation/{group}/{s}/{metric}"] = nan_to_0(PRED_METRICS_LIST[metric](y_true_sub, y_pred_sub))
                        
                        for metric in PRED_METRICS_LIST_BALANCED:
                            self.run_neptune[f"evaluation/{group}/{s}/{metric}"] = nan_to_0(PRED_METRICS_LIST_BALANCED[metric](y_true_sub, y_pred_sub))


                    # Computing clustering metrics
                    for metric in CLUSTERING_METRICS_LIST:
                        self.run_neptune[f"evaluation/{group}/{metric}"] = CLUSTERING_METRICS_LIST[metric](enc, y_pred)
                    ...
                    if group == 'full':
                        ...
                        self.run_neptune[f'evaluation/{group}/latent_space'].track_files(save_dir + f'latent_space_{group}.npy')
                        self.run_neptune[f'evaluation/{group}/predictions'].track_files(save_dir + f'predictions_{group}.csv')
                        ...
                        self.run_neptune[f'evaluation/{group}/umap'].track_files(save_dir + f'umap_{group}.npy')
                        ...
                        self.run_neptune[f'evaluation/{group}/batch_umap'].upload(fig_batch)
                        self.run_neptune[f'evaluation/{group}/split_umap'].upload(fig_split)
    ...
    def train_scheme(self, training_scheme,
        ...
        for (strategy, n_epochs, use_perm) in training_scheme:
            optimizer = self.get_optimizer(self.run_file.learning_rate, self.run_file.weight_decay, self.run_file.optimizer_type) # resetting optimizer state when switching strategy
            if verbose :
    ...
    def training_loop(self, history,
        ...
        for step in range(1, n_steps + 1):
            ...
            with tf.GradientTape() as tape:
                ...
                if training_strategy == "full_model":
                    loss = tf.add_n([self.run_file.clas_w * clas_loss] + 
                                    [self.run_file.dann_w * dann_loss] + [self.run_file.rec_w * rec_loss] + ae.losses)
                elif training_strategy == "warmup_dann":
                    loss = tf.add_n([self.run_file.dann_w * dann_loss] + 
                                    [self.run_file.rec_w * rec_loss] + ae.losses)
                elif training_strategy == "warmup_dann_no_rec":
                    loss = tf.add_n([self.run_file.dann_w * dann_loss] + ae.losses)
                elif training_strategy == "dann_with_ae":
                    loss = tf.add_n([self.run_file.dann_w * dann_loss] + [self.run_file.rec_w * rec_loss] + ae.losses)
                elif training_strategy == "classifier_branch":
    ...
    def evaluation_pass(self,history, ae, adata_list, X_list, y_list, batch_list, clas_loss_fn, dann_loss_fn, rec_loss_fn):
        ...
        #         return _, clas, dann, rec
                ...
                history[group]['rec_loss'] += [rec_loss] """
                history[group]['total_loss'] += [self.run_file.clas_w * clas_loss + self.run_file.dann_w * dann_loss 
                                                 + self.run_file.rec_w * rec_loss + ae.losses] # using numpy to prevent memory leaks
    ...
    def get_scheme(self):
        if self.run_file.training_scheme == 'training_scheme_1':
            self.run_file.training_scheme = [("warmup_dann", self.run_file.warmup_epoch, False),
        ...
        if self.run_file.training_scheme == 'training_scheme_3':
            self.run_file.training_scheme = [("permutation_only", 100, True),  # This will end with a callback
    ...
    def get_losses(self):
        ...
        if self.run_file.clas_loss_name == 'categorical_crossentropy':
            ...
            print(self.run_file.clas_loss_name + ' loss not supported for classif')
        
        if self.run_file.dann_loss_name == 'categorical_crossentropy':
            self.dann_loss_fn = tf.keras.losses.categorical_crossentropy
        else:
            print(self.run_file.dann_loss_name + ' loss not supported for dann')
