...
def harmony_svm(X_list, y_list, batch_list, assign, adata_list, pred_full = True):# -> tuple[dict[Any, Any], dict[Any, Any]]: 
...
def celltypist_model(X_list, y_list, batch_list, assign, adata_list,n_jobs=30, pred_full = True):# -> tuple:
    """ Perform label transfer using CellTypist :
        - latent = X_pca
        - y_pred = prediction for all cells"""
    adata = sc.AnnData(X = X_list['full'].copy(),
                       obs = pd.DataFrame({
    ...
    X_pca_list = {group : X_pca[assign == group, :] for group in np.unique(assign)}
    X_pca_list['full'] = X_pca
    y_pred_list =  {group : predictions.predicted_labels['predicted_labels'][assign == group] for group in np.unique(assign)}
    y_pred_list['full'] = predictions.predicted_labels['predicted_labels']
    return X_pca_list, y_pred_list

def scmap_cluster(X_list, y_list, batch_list, assign, adata_list, pred_full = True):
    adata = sc.AnnData(X = densify(X_list['full']),
                       obs = pd.DataFrame({'celltype': y_list['full']},
                                          index =  y_list['full'].index))

    adata.var['Gene_names'] = adata.var_names
    adata_train = adata[assign == 'train',:]
    adata_test_val = adata[assign.isin(['test', 'val']),:]
        
    adata_train,adata_test_val = common_genes(adata_train,adata_test_val,"Gene_names",remove_unmached=True)
    
    sc.pp.highly_variable_genes(adata_train)
    
    y_pred = scmap_annotate(adata_test_val,adata_train, # train is the ref, test_val is the query
               "Gene_names","celltype",inplace = False,
               algorithm_flavor="centroid",gene_selection_flavor="HVGs",
               similarity_threshold=.7, key_added="scmap_annotation")
    y_pred = pd.Series(y_pred, index = adata_test_val.obs_names)
    y_pred = pd.concat([y_pred, y_list['train']])
    y_pred = y_pred[y_list['full'].index] # reordering them according to y_full just to be sure
...
def scmap_cells(X_list, y_list, batch_list, assign, adata_list, pred_full = True):
    adata = sc.AnnData(X = densify(X_list['full']),
                       obs = pd.DataFrame({'celltype': y_list['full']},
                                          index =  y_list['full'].index))
    ...
    y_pred = scmap_annotate(adata_test_val,adata_train, # train is the ref, test_val is the query
               "Gene_names","celltype",inplace = False,
               algorithm_flavor="cell",gene_selection_flavor="HVGs",
               similarity_threshold=.7, key_added="scmap_annotation")
    y_pred = pd.Series(y_pred, index = adata_test_val.obs_names)
    y_pred = pd.concat([y_pred, y_list['train']]) 
    y_pred = y_pred[y_list['full'].index] # reordering them according to y_full just to be sure
...
def uce(X_list,y_list,batch_list, assign, adata_list, pred_full = True):
    ...
    X_uce_list = {group : X_uce[assign == group, :] for group in np.unique(assign)}
...
def scanvi(X_list, y_list, batch_list, assign, adata_list):
    ...
    adata = sc.AnnData(X = X_list['full'].copy(),
                       obs = pd.DataFrame({
                           ...
                           'batch': batch_list['full'].copy(),
                           'split': assign.copy()}, index = assign.index))
    ...
    latent_list = {group : np.asarray(adata[adata.obs['split'] == group, :].obsm[SCANVI_LATENT_KEY]) for group in np.unique(assign)}
    latent_list['full'] = adata.obsm[SCANVI_LATENT_KEY]
    y_pred_list =  {group : np.asarray(adata[adata.obs['split'] == group, :].obs[SCANVI_PREDICTION_KEY]) for group in np.unique(assign)}
