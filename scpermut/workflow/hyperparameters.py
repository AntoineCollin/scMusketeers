...
from tools.utils import default_value, scanpy_to_input, str2bool
...
class Workflow:
    def __init__(self, run_file, working_dir):
        ...
        self.rec_loss_name = default_value(
            self.rec_loss_name, "categorical_crossentropy"
        )
        ...
        self.ae_output_activation = default_value(
            self.ae_output_activation, "linear"
        )
    ...
    def make_experiment(self, params):
        ...
        if self.log_neptune:
            ...
            self.run[f"parameters/ae_hidden_size"] = stringify_unsupported(
                self.ae_hidden_size
            )
        # Creation of model
            ...
            for group in ["full", "train", "val", "test"]:
                with tf.device("CPU"):
                    ...
                    labels = list(
                        set(
                            np.unique(
                                adata_list[group].obs[f"true_{self.class_key}"]
                            )
                        ).union(
                            set(
                                np.unique(
                                    self.dataset.ohe_celltype.inverse_transform(
                                        clas
                                    )
                                )
                            )
                    ...
                    if group == "full":
                        ...
                        self.run[f"evaluation/{group}/classif_umap"].upload(
                            fig_class
                        )
        ...
        with tf.device("CPU"):
            ...
            clas = np.eye(clas.shape[1])[np.argmax(clas, axis=1)]
            opt_metric = self.pred_metrics_list["mcc"](
                np.asarray(y_list["val"].argmax(axis=1)), clas.argmax(axis=1)
            )  # We retrieve the last metric of interest
        if self.log_neptune:
            self.run.stop()
        del enc
        del clas
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset_name",
        type=str,
        default="disco_ajrccm_downsampled",
        help="Name of the dataset to use, should indicate a raw h5ad AnnData file",
    )
    parser.add_argument(
        "--class_key",
        type=str,
        default="celltype_lv2_V3",
        help="Key of the class to classify",
    )
    parser.add_argument(
        "--batch_key", type=str, default="manip", help="Key of the batches"
    )
    parser.add_argument(
        "--filter_min_counts",
        type=str2bool,
    ...
    parser.add_argument(
        "--pct_false", type=float, nargs="?", default=0, help=""
    )
    parser.add_argument(
        "--clas_loss_name",
        type=str,
    ...
    parser.add_argument(
        ...
        nargs="?",
        default=0.1,
        help="Wight of the classification loss",
    )
    parser.add_argument(
        "--dann_w",
        type=float,
    ...
    parser.add_argument(
        ...
        nargs="?",
        default=0.8,
        help="Wight of the reconstruction loss",
    )
    parser.add_argument(
        "--warmup_epoch",
        type=float,
    ...
    parser.add_argument(
        "--ae_hidden_dropout", type=float, nargs="?", default=0, help=""
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--ae_l1_enc_coef", type=float, nargs="?", default=0, help=""
    )
    parser.add_argument(
        "--ae_l2_enc_coef", type=float, nargs="?", default=0, help=""
    )
    parser.add_argument(
        "--class_hidden_size",
        type=int,
    ...
    parser.add_argument(
        "--class_hidden_dropout", type=float, nargs="?", default=0, help=""
    )
    parser.add_argument(
        "--class_batchnorm",
        type=str2bool,
    ...
    parser.add_argument(
        "--dann_hidden_dropout", type=float, nargs="?", default=0, help=""
    )
    parser.add_argument(
        "--dann_batchnorm",
        type=str2bool,
    ...
    def train_cmd(params):
        ...
        (
            run_file.dann_hidden_dropout,
            run_file.class_hidden_dropout,
            run_file.ae_hidden_dropout,
        ) = (dropout, dropout, dropout)

        cmd = [
            "sbatch",
            "--wait",
            "/home/simonp/dca_permuted_workflow/workflow/run_workflow_cmd.sh",
        ]
